...
%задаем доп данные для вывода
    f_id = fopen('ne_16_node_test_drob.txt', 'w');
    t_id = fopen('ne_16_node_test_stepmin_drob.txt', 'w');
...
%запихиваем все параметры метода в одну структуру 
     ...
     valueSet = { x,             out{1}, out{2}, out{3},1e-6,         0.01,    f_id,   iter_format, 'first',        'no', 'drob'};
...
%делаем вывод в файл заголовка в зависимости от варианта вывода
     if strcmp(params('output_format'), 'first')
        fprintf(f_id, '%6s %15s %15s %10s %15s', 'iter', 'grad(x_k)', 'value', 'Step', 'lambda');   
